...
interface EmailCopy {
  ...
  kicker:          string
  headline:        string
  crawlBudgetLabel: string
  urlsChecked:     (n: string) => string
  barOk:           string
  barRedirects:    string
  barErrors:       string
  barLoops:        string
  ...
  catErrors:       string
  catRedirects:    string
  actionLoop:      (n: number) => string
  actionErrors:    (n: number) => string
  actionRedirects: (n: number) => string
}

const COPY: Record<Locale, EmailCopy> = {
  pt: {
    htmlLang:         'pt-BR',
    kicker:           'REDIRECT CHECKER',
    headline:         'Seu relatório de URLs está pronto.',
    crawlBudgetLabel: 'Crawl budget afetado',
    urlsChecked:      (n) => `${n} URLs verificadas`,
    barOk:            'OK — sem redirect',
    barRedirects:     'Redirects',
    barErrors:        'Erros (4xx / 5xx / timeout)',
    barLoops:         'Loops de redirecionamento',
    urlsSuffix:       ' URLs',
    affected:         'afetado',
    prioritiesLabel:  'Ações prioritárias',
    noIssues:         '🎉 Nenhum problema crítico detectado. Seu sitemap está saudável!',
    ctaButton:        'Ver relatório completo →',
    footerLine:       'dup.agency — 2026',
    catLoop:          'Loop',
    catErrors:        'Erros',
    catRedirects:     'Redirects',
    actionLoop:       (n) => `${n} loop${n > 1 ? 's' : ''} detectado${n > 1 ? 's' : ''}. Revise as regras de redirecionamento — loops bloqueiam o Googlebot completamente.`,
    actionErrors:     (n) => `${n} URL${n > 1 ? 's' : ''} com erro (4xx/5xx/timeout). Corrija ou remova do sitemap para não desperdiçar crawl budget.`,
    actionRedirects:  (n) => `${n} URL${n > 1 ? 's' : ''} com redirect. Encurte as cadeias para economizar crawl budget e melhorar a velocidade de indexação.`,
  },
  en: {
    htmlLang:         'en',
    kicker:           'REDIRECT CHECKER',
    headline:         'Your URL report is ready.',
    crawlBudgetLabel: 'Crawl budget affected',
    urlsChecked:      (n) => `${n} URLs checked`,
    barOk:            'OK — no redirect',
    barRedirects:     'Redirects',
    barErrors:        'Errors (4xx / 5xx / timeout)',
    barLoops:         'Redirect loops',
    urlsSuffix:       ' URLs',
    affected:         'affected',
    prioritiesLabel:  'Priority actions',
    noIssues:         '🎉 No critical issues found. Your sitemap is healthy!',
    ctaButton:        'See full report →',
    footerLine:       'dup.agency — 2026',
    catLoop:          'Loop',
    catErrors:        'Errors',
    catRedirects:     'Redirects',
    actionLoop:       (n) => `${n} loop${n > 1 ? 's' : ''} detected. Review your redirect rules — loops block Googlebot completely.`,
    actionErrors:     (n) => `${n} URL${n > 1 ? 's' : ''} with errors (4xx/5xx/timeout). Fix or remove them from the sitemap so you don't waste crawl budget.`,
    actionRedirects:  (n) => `${n} URL${n > 1 ? 's' : ''} with redirects. Shorten the chains to save crawl budget and speed up indexing.`,
  },
  es: {
    htmlLang:         'es',
    kicker:           'REDIRECT CHECKER',
    headline:         'Tu reporte de URLs está listo.',
    crawlBudgetLabel: 'Crawl budget afectado',
    urlsChecked:      (n) => `${n} URLs verificadas`,
    barOk:            'OK — sin redirect',
    barRedirects:     'Redirects',
    barErrors:        'Errores (4xx / 5xx / timeout)',
    barLoops:         'Loops de redirección',
    urlsSuffix:       ' URLs',
    affected:         'afectado',
    prioritiesLabel:  'Acciones prioritarias',
    noIssues:         '🎉 No se detectaron problemas críticos. ¡Tu sitemap está sano!',
    ctaButton:        'Ver reporte completo →',
    footerLine:       'dup.agency — 2026',
    catLoop:          'Loop',
    catErrors:        'Errores',
    catRedirects:     'Redirects',
    actionLoop:       (n) => `${n} loop${n > 1 ? 's' : ''} detectado${n > 1 ? 's' : ''}. Revisá las reglas de redirección — los loops bloquean al Googlebot por completo.`,
    actionErrors:     (n) => `${n} URL${n > 1 ? 's' : ''} con error (4xx/5xx/timeout). Corregilas o quitalas del sitemap para no desperdiciar crawl budget.`,
    actionRedirects:  (n) => `${n} URL${n > 1 ? 's' : ''} con redirect. Acortá las cadenas para ahorrar crawl budget y acelerar la indexación.`,
  },
}
...
function topActions(s: RedirectCheckSummary, c: EmailCopy): ActionItem[] {
  const items: ActionItem[] = []
  if (s.loops > 0)
    items.push({ color: '#C04040', category: c.catLoop, action: c.actionLoop(s.loops) })
  if (s.errors > 0)
    items.push({ color: '#D4A017', category: c.catErrors, action: c.actionErrors(s.errors) })
  if (s.redirects > 0)
    items.push({ color: '#51A899', category: c.catRedirects, action: c.actionRedirects(s.redirects) })
  return items.slice(0, 3)
}

export function buildEmailHtml(summary: RedirectCheckSummary, resultUrl: string, locale: string = 'pt'): string {
  const c      = dict(locale)
  const pct    = crawlBudgetPct(summary)
  const col    = crawlColor(pct)
  const items  = topActions(summary, c)
  ...
  const bar = (label: string, value: number, color: string) => {
    ...
    return `<tr><td style="padding-bottom:16px">
      <table width="100%" cellpadding="0" cellspacing="0"><tr>
        ...
        <td align="right"><p style="margin:0;font-size:13px;font-weight:700;color:${color}">${value}<span style="font-size:11px;font-weight:400;color:#bbb">${c.urlsSuffix}</span></p></td>
      </tr></table>
      ...
      <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:16px;overflow:hidden;border:1px solid #e8e8e8">
          ...
          <table cellpadding="0" cellspacing="0"><tr>
            <td style="padding-right:24px;vertical-align:middle">
              <svg width="120" height="120" viewBox="0 0 128 128" xmlns="http://www.w3.org/2000/svg">
                ...
                <text x="64" y="76" text-anchor="middle" font-size="11" fill="#bbb">${c.affected}</text>
              </svg>
            </td>
            <td style="vertical-align:middle">
              <p style="margin:0 0 4px;font-size:11px;color:#bbb;text-transform:uppercase;letter-spacing:0.08em">${c.crawlBudgetLabel}</p>
              ...
              <p style="margin:0;font-size:12px;color:#676767">${c.urlsChecked(summary.totalUrls.toLocaleString(LOCALE_TAG[(locale as Locale)] ?? 'pt-BR'))}</p>
            </td>
          </tr></table>
        </td></tr>
        ...
        <tr><td style="padding:0 40px 28px">
          <table width="100%" cellpadding="0" cellspacing="0">
            ${bar(c.barOk, summary.ok, '#3D9688')}
            ${bar(c.barRedirects, summary.redirects, '#897BBC')}
            ${bar(c.barErrors, summary.errors, '#D4A017')}
            ${bar(c.barLoops, summary.loops, '#C04040')}
          </table>
        </td></tr>
        ...
        <tr><td style="padding:0 40px 28px;border-top:1px solid #f0f0f0">
          <p style="margin:24px 0 0;font-size:13px;color:#3d3d3d;text-align:center">${c.noIssues}</p>
        </td></tr>`}
        ...
        <tr><td style="background:#0d0d0d;padding:28px 40px 32px;border-top:1px solid #1a1a1a;text-align:center">
          <table cellpadding="0" cellspacing="0" role="presentation" style="margin:0 auto 16px"><tr>
            ...
            <td style="padding:0 12px"><a href="https://open.spotify.com/playlist/6QH8ouatvWzVQUp52uCZQL?si=fA6XtpCfQ_CMLkLt2QedQg&pi=dNgsB0zUQYizS" target="_blank" rel="noopener noreferrer" style="display:inline-block"><svg width="20" height="20" viewBox="0 0 24 24" fill="#555555" xmlns="http://www.w3.org/2000/svg"><path d="M12 0C5.4 0 0 5.4 0 12s5.4 12 12 12 12-5.4 12-12S18.66 0 12 0zm5.521 17.34c-.24.359-.66.48-1.021.24-2.82-1.74-6.36-2.101-10.561-1.141-.418.122-.779-.179-.899-.539-.12-.421.18-.78.54-.9 4.56-1.021 8.52-.6 11.64 1.32.42.18.479.659.301 1.02zm1.44-3.3c-.301.42-.841.6-1.262.3-3.239-1.98-8.159-2.58-11.939-1.38-.479.12-1.02-.12-1.14-.6-.12-.48.12-1.021.6-1.141C9.6 9.9 15 10.561 18.72 12.84c.361.181.54.78.241 1.02zm.12-3.36C15.24 8.4 8.82 8.16 5.16 9.301c-.6.179-1.2-.181-1.38-.721-.18-.601.18-1.2.72-1.381 4.26-1.26 11.28-1.02 15.721 1.621.539.3.719 1.02.419 1.56-.299.421-1.02.599-1.559.3z"/></svg></a></td>
