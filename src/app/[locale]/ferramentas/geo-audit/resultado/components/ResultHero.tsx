...
export default function ResultHero() {
  ...
  function scoreLabel(s: number) {
    if (s >= 80) return tm('scoreLabelBom')
    if (s >= 60) return tm('scoreLabelRegular')
    if (s >= 40) return t('status.atencao')
    return t('status.critico')
  }

  const keywords = tm.raw('keywords') as string[]
  const topGaps = tm.raw('topGaps') as string[]
  return (
    <section
    ...
    >
      <div style={{ maxWidth: '1040px', margin: '0 auto', position: 'relative' }}>
        ...
        >
          <a href={getPathname({ href: '/ferramentas/geo-audit', locale })} style={{ color: 'var(--purple-mid)', textDecoration: 'none' }}>
            {t('breadcrumb')}
          </a>
        </p>
        ...
        >
          ...
          <div style={{ padding: '40px 40px 48px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '48px', alignItems: 'start' }}>
            ...
            <div>
              ...
              >
                {tm('analiseConcluida')} · {mockResult.analyzedAt}
              </span>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '24px' }}>
              ...
              >
                {[
                  { label: t('seoTecnico'),   score: 48 },
                  { label: t('geoReadiness'), score: 34 },
                ].map((m) => (
                  <div key={m.label}>
